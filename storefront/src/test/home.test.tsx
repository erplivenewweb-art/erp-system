...
describe("Phase 3D static homepage", () => {
  it("renders exactly one H1 and every required editorial region", () => {
    ...
    for (const heading of [
      homeContent.collections.title,
      homeContent.products.title,
      homeContent.manufacturing.title,
      homeContent.workshop.title,
      homeContent.purity.title,
      homeContent.custom.title,
      homeContent.wholesale.title,
      homeContent.packaging.title,
      homeContent.reviews.title,
      homeContent.social.title,
      homeContent.journal.title,
  ...
  it("provides accessible retail and wholesale calls to action", () => {
    render(<Page />);
    expect(
      screen.getByRole("link", { name: homeContent.hero.primary.label }),
    ).toHaveAttribute("href", "/products");
    expect(
      screen.getByRole("link", { name: homeContent.hero.secondary.label }),
    ).toHaveAttribute("href", "/wholesale");
    expect(
      screen.getByRole("link", { name: homeContent.wholesale.action.label }),
    ).toHaveAttribute("href", "/wholesale");
  });

  it("labels every price as simulated and exposes no wholesale price values", () => {
    const html = renderToStaticMarkup(<Page />);
    expect(html).toMatch(/Simulated retail ₹\d/);
    expect(html).not.toMatch(/wholesale (?:price|₹)\s*[:₹]?\s*\d/i);
    expect(html).toContain(
      "No live retail price, wholesale price or stock is shown.",
    );
  });
  ...
  it("labels synthetic products and review placeholders honestly", () => {
    ...
    expect(
      screen.getByText(
        "Synthetic homepage product shells. No live retail price, wholesale price or stock is shown.",
      ),
    ).toBeVisible();
    expect(
      screen.getAllByText("Synthetic placeholder — not a verified review"),
    ).toHaveLength(homeContent.reviews.items.length);
  });
  ...
  it("publishes indexable homepage metadata without inventing a domain", () => {
    ...
    expect(designSystemMetadata.robots).toMatchObject({
      index: false,
      follow: false,
    });
