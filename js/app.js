      ...
      <tr>
        ...
        <td>
          <div class="action-btn-row">
            ...
            <button class="action-btn-small action-delete" type="button" onclick="deleteStickerItem('${item.barcode}')">ðŸ—‘</button>
          </div>
        </td>
      </tr>
    `;
...
function renderHistory() {
  ...
  merged.forEach((item) => {
    const leftButton =
      item.type === "ACTIVE"
        ? `<button type="button" onclick="fillStickerFormByBarcode('${item.barcode}')">â–¶</button>`
        : `<button type="button" onclick="restoreByBarcode('${item.barcode}')">â†º</button>`;

    const rightButton =
      item.type === "ACTIVE"
        ? `<button type="button" title="Delete" onclick="deleteStickerItem('${item.barcode}')">ðŸ—‘</button>`
        : `<button type="button" title="Restore" onclick="restoreByBarcode('${item.barcode}')">â†º</button>`;
...
function sellInvoiceItem() {
  ...
  // ðŸ”¥ IMPORTANT â†’ calculation force
  calculateInvoiceTotal();

  // ðŸ”¥ ensure items me latest values ho
...
function renderSalesAnalytics() {
  ...
  fetch(`${API_BASE}/getSales`)
    ...
    .then((data) => {
      ...
      if (totalSalesAmountEl) totalSalesAmountEl.textContent = "â‚¹ " + totalSalesAmount.toFixed(2);
      if (lowStockItemsEl) lowStockItemsEl.textContent = lowStockCount;
    })
    .catch((err) => {
